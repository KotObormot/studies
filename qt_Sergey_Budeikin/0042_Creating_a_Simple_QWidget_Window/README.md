...
Отличные новости! На скриншоте из Looking Glass мы видим идеальную картину: статус "untracked" пропал, и теперь там четко написано app: simple_qwidget.desktop, а рядом отображается ваша цветная иконка. Это значит, что оконный менеджер Mutter (Wayland) полностью распознал приложение и правильно связал его с панелью задач (Dock) и меню Alt+Tab.

То, что сама картинка не отображается в левом верхнем углу рамки самого окна, — это абсолютно нормальное, стандартное поведение для современной Ubuntu.
...
Great news! The screenshot from Looking Glass shows a perfect picture: the "untracked" status has disappeared, and it now clearly says app: simple_qwidget.desktop, with your colored icon displayed next to it. This means the Mutter (Wayland) window manager has fully recognized the application and correctly associated it with the taskbar (Dock) and the Alt+Tab menu.

The fact that the image itself isn't displayed in the upper-left corner of the window frame is completely normal, standard behavior for modern Ubuntu.
